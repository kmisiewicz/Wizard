...
bool TextureManager::LoadTexture(std::string nameID, std::string filepath, GLenum magFilter, GLenum minFilter)
{
	...
	if (minFilter == GL_LINEAR_MIPMAP_LINEAR || minFilter == GL_LINEAR_MIPMAP_NEAREST) {
		gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, width, height, GL_BGR_EXT, GL_UNSIGNED_BYTE, bits);
	}
	else {
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, bits);
	}
...
bool TextureManager::BindTexture(std::string texID)
{
	bool result(true);
