...
Staff::Staff(vec3 pos, vec3 color, std::string objectType, Player * player, float baserotate)
{
	this->pos = pos;
	this->color = color;
...
bool Staff::load(std::string filename)
{
	struct SFace {
		int v[3];
		int n[3];
...
void Staff::Render()
{
	...
	glPushMatrix();
		...
		glRotatef(attackrotate/1.5, 0, 0, 1);
		glRotatef(rotate2 + 90 - attackrotate, 1, 0, 0);
...
void Staff::Update()
{
	...
	rotate = -atan2(player->dir.z, player->dir.x) * 180 / PI;
	rotate2 = -acos(player->dir.y) * 180 / PI;
